...
contract ComplexRewarderTime is IRewarder,  Ownable{
    ...
    constructor (IERC20 _rewardToken, uint256 _rewardPerSecond, address _NUCODE_CHEF) {
        rewardToken = _rewardToken;
        rewardPerSecond = _rewardPerSecond;
        NUCODE_CHEF = _NUCODE_CHEF;
    }
    ...
    function pendingToken(uint256 _pid, address _user) public view returns (uint256 pending) {
        ...
        uint256 lpSupply = IMiniChefV2(NUCODE_CHEF).lpToken(_pid).balanceOf(NUCODE_CHEF);
    ...
    function updatePool(uint256 pid) public returns (PoolInfo memory pool) {
        ...
        if (block.timestamp > pool.lastRewardTime) {
            uint256 lpSupply = IMiniChefV2(NUCODE_CHEF).lpToken(pid).balanceOf(NUCODE_CHEF);
